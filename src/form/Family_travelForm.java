...
public class Family_travelForm extends JFrame  {
	 ...
	 */
	public Family_travelForm(Connection connection) throws SQLException {
		...
		Travel a = new Travel();
		travel = new ArrayList<>(a.getTable(connection));
		comboBoxAccessories.removeAllItems();
		for (int i = 0; i < travel.size(); i++) {
			comboBoxAccessories.addItem("" + travel.get(i).getBreakdown());
		}
	...
	public Family_travelForm(int id, Connection connection) throws SQLException {
		...
		ArrayList<Family_travel> dev_acc = new ArrayList<>(da.getTable(connection));
		da = null;
		...
		for (int i = 0; i < travel.size(); i++) {
			comboBoxAccessories.addItem("" + travel.get(i).getBreakdown());
			if(da.getDeviceId() == travel.get(i).getId()){
				aid = i;
			}
		}
		comboBoxDevice.setSelectedItem(family.get(did).getFullName());
		comboBoxAccessories.setSelectedItem(travel.get(aid).getBreakdown());
	}
	...
	private void initialize() {
		...
		buttonSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Family_travel device_accessories = null;
				
				try {
					device_accessories = new Family_travel( (comboBoxDevice.getSelectedIndex()),
							(comboBoxAccessories.getSelectedIndex()));
					if (idSelected < 0) {
						device_accessories.addElement(
								family.get(comboBoxDevice.getSelectedIndex()).getId(),
								travel.get(comboBoxAccessories.getSelectedIndex())
										.getId(), connection);
					} else {
					...
					Logger.getLogger(Family_travelForm.class.getName()).log(
