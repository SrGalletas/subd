...
public class Family_housingForm extends JFrame  {
	...
	private ArrayList<Housing> housing;

	/**
	 * @wbp.parser.constructor
	 */
	public Family_housingForm(Connection connection) throws SQLException {
		...
		housing = new ArrayList<>(a.getTable(connection));
		comboBoxAccessories.removeAllItems();
		for (int i = 0; i < housing.size(); i++) {
			comboBoxAccessories.addItem("" + housing.get(i).getFullName());
		}
	...
	public Family_housingForm(int id, Connection connection) throws SQLException {
		...
		ArrayList<Family_housing> dev_acc = new ArrayList<>(da.getTable(connection));
		da = null;
		...
		for (int i = 0; i < housing.size(); i++) {
			comboBoxAccessories.addItem("" + housing.get(i).getFullName());
			if(da.getDeviceId() == housing.get(i).getId()){
				aid = i;
			}
		}
		comboBoxDevice.setSelectedItem(family.get(did).getFullName());
		comboBoxAccessories.setSelectedItem(housing.get(aid).getFullName());
	}
	...
	private void initialize() {
		...
		buttonSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Family_housing device_accessories = null;
				
				try {
					device_accessories = new Family_housing( (comboBoxDevice.getSelectedIndex()),
							(comboBoxAccessories.getSelectedIndex()));
					if (idSelected < 0) {
						device_accessories.addElement(
								family.get(comboBoxDevice.getSelectedIndex()).getId(),
								housing.get(comboBoxAccessories.getSelectedIndex())
										.getId(), connection);
					} else {
					...
					Logger.getLogger(Family_housingForm.class.getName()).log(
